...
2. [PARTIE 1 : STRUCTURE DÉTAILLÉE PAR "KEYSPACE" LOGIQUE](#-partie-1--structure-détaillée-par-keyspace-logique)
3. [PARTIE 2 : MIGRATION VERS HCD](#-partie-2--migration-vers-hcd)
4. [PARTIE 3 : BESOINS À DÉMONTRER](#-partie-3--besoins-à-démontrer)
5. [PARTIE 4 : RÉSUMÉ DES BESOINS](#-partie-4--résumé-des-besoins)
...
- `11_test_feedbacks_counters.sh` - Tests compteurs feedbacks par libellé
- `12_test_historique_opposition.sh` - Tests historique oppositions
- `25_test_feedbacks_ics.sh` - Tests feedbacks par ICS
- `26_test_decisions_salaires.sh` - Tests décisions salaires
- `15_test_coherence_multi_tables.sh` - Tests cohérence multi-tables

**Documents Associés** :
- `12_ANALYSE_DETAIL_DEMONSTRATIONS_META_CATEGORIES.md` - Analyse détaillée
