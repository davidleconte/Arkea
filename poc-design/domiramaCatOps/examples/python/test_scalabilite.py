    ...
    if perf:
        ...
        for volume in volumes:
            if volume > current_count:
                ...
                estimated_latency = perf['avg'] * (volume / current_count) ** 0.5  # Approximation sqrt
...
def test_scalabilite_index(session, code_si: str, contrat: str):
    ...
    try:
        ...
        print(f"   ✅ Limite SAI : 10 index (par défaut)")
        print(f"   ✅ Utilisation : {index_count}/10 ({index_count*10}%)")
        
        if index_count < 10:
            print(f"   ✅ Marge disponible : {10 - index_count} index")
        else:
            print(f"   ⚠️  Limite atteinte : Aucun index supplémentaire possible")
...
def test_degradation_performance(session, code_si: str, contrat: str):
    ...
    for level in concurrent_levels:
        degradation = ((results[level]['avg_latency'] - base_latency) / base_latency * 100) if base_latency > 0 else 0
