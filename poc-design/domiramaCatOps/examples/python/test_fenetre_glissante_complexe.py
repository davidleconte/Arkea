...
def get_operations_count(session, code_si: str, contrat: str, start_date: str, end_date: str) -> int:
...
def test_fenetre_glissante_avec_chevauchement(session, code_si: str, contrat: str):
    ...
    print(f"   ✅ Fenêtres avec chevauchement validées")
...
def test_fenetre_glissante_sans_chevauchement(session, code_si: str, contrat: str):
    ...
    print(f"   ✅ Fenêtres sans chevauchement validées")
...
def test_gestion_frontieres(session, code_si: str, contrat: str):
    ...
    try:
        ...
        if first_date and last_date:
            ...
            count_before = get_operations_count(session, code_si, contrat, before_first, first_date.strftime('%Y-%m-%d'))
            print(f"   ✅ Fenêtre avant première date : {count_before} opérations (devrait être 0 ou faible)")
            
            # Test dernière fenêtre (après dernière date)
            after_last = (last_date + timedelta(days=5)).strftime('%Y-%m-%d')
            count_after = get_operations_count(session, code_si, contrat, last_date.strftime('%Y-%m-%d'), after_last)
            print(f"   ✅ Fenêtre après dernière date : {count_after} opérations (devrait être 0 ou faible)")
            
            return True, f"Première: {first_date.strftime('%Y-%m-%d')}, Dernière: {last_date.strftime('%Y-%m-%d')}"
        else:
...
def test_aggregation_multi_fenetres(session, code_si: str, contrat: str):
    ...
    stats['min_operations'] = stats['min_operations'] if stats['min_operations'] != float('inf') else 0
    stats['avg_operations'] = stats['total_operations'] / stats['total_fenetres'] if stats['total_fenetres'] > 0 else 0
    
    print(f"   ✅ Total opérations : {stats['total_operations']}")
    print(f"   ✅ Fenêtres avec données : {stats['fenetres_avec_donnees']}/{stats['total_fenetres']}")
...
def test_fenetre_glissante_complexe():
    ...
    counts_chevauchement = test_fenetre_glissante_avec_chevauchement(session, test_code_si, test_contrat)
    results['chevauchement'] = counts_chevauchement
    
    # Test 2 : Fenêtre sans chevauchement
    counts_sans_chevauchement, total_expected = test_fenetre_glissante_sans_chevauchement(session, test_code_si, test_contrat)
