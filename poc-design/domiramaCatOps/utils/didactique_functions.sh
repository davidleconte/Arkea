...
check_schema() {
    ...
    if [ -n "$column" ]; then
        column_check=$("$cqlsh_bin" localhost 9042 -e "USE domiramacatops_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -c "$column" || echo "0")
    ...
    if [ -n "$index" ]; then
        index_check=$("$cqlsh_bin" localhost 9042 -e "USE domiramacatops_poc; DESCRIBE INDEX $index;" 2>&1 | grep -c "$index" || echo "0")
...
# ============================================
# FONCTION : Vérifier le Statut HCD
# ============================================
#
# Usage: check_hcd_status
#
check_hcd_status() {
    ...
    # Vérifier que le port 9042 est ouvert (alternative à cqlsh si six n'est pas disponible)
...
# ============================================
# FONCTION : Vérifier la Version Java via jenv
# ============================================
#
# Usage: check_jenv_java_version
#
check_jenv_java_version() {
    if ! command -v jenv &> /dev/null; then
        warn "jenv n'est pas installé. Java peut ne pas être configuré correctement."
        return 0
    fi
    ...
    if jenv versions | grep -q "11"; then
        ...
        warn "Java 11 non trouvé dans jenv. Vérifiez la configuration."
    fi
}

# ============================================
# FONCTION : Exécuter une Requête CQL (Version Simplifiée)
# ============================================
#
# Usage: execute_cql_query "$cql_query" "Description"
#
execute_cql_query() {
    local query="$1"
    local description="${2:-Exécution requête CQL}"
    local cqlsh_bin="${HCD_HOME:-${HCD_DIR}}/bin/cqlsh"
...
# ============================================
# FONCTION : Afficher un DDL Section Formaté
# ============================================
#
# Usage: show_ddl_section "$ddl_content"
#
show_ddl_section() {
    local ddl="$1"
    echo ""
...
# ============================================
# FONCTION : Configuration des Chemins (Portable)
# ============================================
#
# Usage: setup_paths
#
# Priorité : Variables d'environnement > .poc-config.sh > Détection automatique
#
setup_paths() {
    ...
    # Priorité 2: Détection automatique (2 niveaux au-dessus de domiramaCatOps)
