...
if [ -f "${BIC_DIR}/utils/didactique_functions.sh" ]; then
    ...
    export SPARK_HOME="${SPARK_HOME:-podman}"
fi

# OSS5.0 Podman mode detection
if [ "$SPARK_HOME" = "podman" ]; then
    PODMAN_SPARK=true
    if ! podman ps --filter "name=arkea-spark" --format "{{.Names}}" 2>/dev/null | grep -q "arkea-spark"; then
        echo "ERROR: Spark container not running. Run 'make demo' first."
        exit 1
    fi
    echo "✅ Mode Podman: Spark container detected"
else
    PODMAN_SPARK=false
fi
...
if [ "$PODMAN_SPARK" = "true" ]; then
    # Podman mode: copy to container and run
    podman cp "$SCALA_TEMP" arkea-spark-master:/tmp/scala_script.scala
    podman exec -it arkea-spark-master spark-shell --conf spark.sql.warehouse.dir=/tmp/spark-warehouse < /tmp/scala_script.scala
else
