...
def concurrent_read_test(
    ...
    print(
        "📋 TEST 1 : Charge Lecture Concurrente "
        f"({num_threads} threads, {queries_per_thread} requêtes/thread)"
    )
    print("-" * 70)
...
def concurrent_write_test(
    ...
    print(
        "\n📋 TEST 2 : Charge Écriture Concurrente "
        f"({num_threads} threads, {inserts_per_thread} insertions/thread)"
    )
    print("-" * 70)
...
def test_charge_concurrente():
    ...
    for result in results:
        if result["type"] == "read":
            print(
                "✅ Lecture : "
                f"{result['throughput']:.2f} req/s, p95={result['latencies']['p95']:.2f}ms"
            )
        elif result["type"] == "write":
            print(
                "✅ Écriture : "
                f"{result['throughput']:.2f} inserts/s, p95={result['latencies']['p95']:.2f}ms"
            )
