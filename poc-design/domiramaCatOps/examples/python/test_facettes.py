...
def test_facettes_temporelles(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        return True, f"Facettes temporelles validées", facettes
    except Exception as e:
        return False, f"Erreur : {e}", {}
...
def test_performance_facettes(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        start = time.time()
        session.execute(f"SELECT cat_auto FROM {KEYSPACE}.operations_by_account WHERE code_si = '{code_si}' AND contrat = '{contrat}'")
        latencies.append(("catégorie", (time.time() - start) * 1000))
        
        # Test 2 : Facettes temporelles
        start = time.time()
        session.execute(f"SELECT date_op FROM {KEYSPACE}.operations_by_account WHERE code_si = '{code_si}' AND contrat = '{contrat}'")
        latencies.append(("temporelle", (time.time() - start) * 1000))
        
        # Test 3 : Facettes combinées
        start = time.time()
        session.execute(f"SELECT cat_auto, date_op FROM {KEYSPACE}.operations_by_account WHERE code_si = '{code_si}' AND contrat = '{contrat}'")
