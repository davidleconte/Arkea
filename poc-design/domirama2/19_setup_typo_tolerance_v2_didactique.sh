...
if [ "$COLUMN_EXISTS" -eq 0 ]; then
    ...
    # Vérification
    COLUMN_CHECK=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep "libelle_prefix" | head -1)
...
INDEX_CHECK=$(./bin/cqlsh localhost 9042 -e "SELECT index_name, index_type FROM system_schema.indexes WHERE keyspace_name = 'domirama2_poc' AND index_name = 'idx_libelle_prefix_ngram';" 2>&1 | grep -v "Warnings" | grep -v "index_name" | grep -v "---" | grep -v "^$")
...
COLUMN_EXISTS_CHECK=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -c "libelle_prefix" || echo "0")
if [ "$COLUMN_EXISTS_CHECK" -gt 0 ]; then
    success "✅ Colonne 'libelle_prefix' existe"
    echo ""
    result "📊 Détails de la colonne :"
    COLUMN_DETAILS=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep "libelle_prefix" | head -1)
    echo "   ┌─────────────────────────────────────────────────────────┐"
...
INDEX_EXISTS_CHECK=$(./bin/cqlsh localhost 9042 -e "SELECT index_name FROM system_schema.indexes WHERE keyspace_name = 'domirama2_poc' AND index_name = 'idx_libelle_prefix_ngram';" 2>&1 | grep -v "Warnings" | grep -c "idx_libelle_prefix_ngram" || echo "0")
if [ "$INDEX_EXISTS_CHECK" -gt 0 ]; then
    success "✅ Index 'idx_libelle_prefix_ngram' existe"
    echo ""
    result "📊 Détails de l'index :"
    INDEX_DETAILS=$(./bin/cqlsh localhost 9042 -e "SELECT index_name, index_type FROM system_schema.indexes WHERE keyspace_name = 'domirama2_poc' AND index_name = 'idx_libelle_prefix_ngram';" 2>&1 | grep -v "Warnings" | grep -v "index_name" | grep -v "---" | grep -v "^$")
    echo "   ┌─────────────────────────────────────────────────────────┐"
...
DATA_CHECK=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT COUNT(*) as total, COUNT(libelle_prefix) as avec_prefix FROM operations_by_account;" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]+" | head -1)
