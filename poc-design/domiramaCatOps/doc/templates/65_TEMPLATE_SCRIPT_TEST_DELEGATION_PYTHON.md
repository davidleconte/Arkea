...
# cqlsh -e "DESCRIBE KEYSPACE domiramacatops_poc" > /dev/null 2>&1
# if [ $? -ne 0 ]; then
...
# Exemple de vérification
# COLUMNS_CHECK=$(cqlsh -e "SELECT column_name FROM system_schema.columns WHERE keyspace_name='domiramacatops_poc' AND table_name='operations_by_account'" 2>/dev/null | grep -c "[colonne]" || echo "0")
