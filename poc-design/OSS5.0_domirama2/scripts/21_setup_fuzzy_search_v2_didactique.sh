...
COLUMN_DETAILS=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -A 2 "libelle_embedding" | grep -v "Warnings" | head -3)
...
sleep 2
INDEX_EXISTS=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "USE domirama2_poc; DESCRIBE INDEX idx_libelle_embedding_vector;" 2>&1 | grep -v "Warnings" | grep -c "idx_libelle_embedding_vector" || echo "0")

if [ "$INDEX_EXISTS" -gt 0 ]; then
    success "✅ Index idx_libelle_embedding_vector vérifié"
    echo ""
    result "📊 Détails de l'index :"
    INDEX_DETAILS=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "USE domirama2_poc; DESCRIBE INDEX idx_libelle_embedding_vector;" 2>&1 | grep -v "Warnings" | head -10)
    echo "   ┌─────────────────────────────────────────────────────────┐"
...
SCHEMA_CHECK=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -E "(libelle_embedding|VECTOR)" | grep -v "Warnings" | head -5)
