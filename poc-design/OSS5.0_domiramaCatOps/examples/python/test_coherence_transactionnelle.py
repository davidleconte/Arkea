...
def test_referential_coherence(session, code_si: str, contrat: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if ops_count > 0:
            print(
                f"   ✅ Cohérence référentielle : operations_by_account existe ({ops_count} opérations)"
            )
...
def test_counter_coherence(session) -> Tuple[bool, str]:
    ...
    try:
        ...
        # et de comparer avec le compteur, ce qui nécessiterait une table de
        # feedbacks détaillée
...
def test_rules_coherence(session) -> Tuple[bool, str]:
    ...
    # Note: HCD ne supporte pas IS NOT NULL ni SELECT DISTINCT, on récupère
    # toutes les catégories et on filtre côté client
    query_ops = """
    SELECT cat_auto
    FROM {KEYSPACE}.operations_by_account
    LIMIT 100
    ...
    try:
        ...
        # Vérifier chaque cat_auto dans regles_personnalisees (limiter à 10
        # pour performance)
        missing_cats = []
        ...
        if not missing_cats:
            print(
                "   ✅ Cohérence règles : toutes les catégories trouvées dans regles_personnalisees"
            )
