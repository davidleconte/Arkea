...
if [ -f "XX_setup_schema.sh" ]; then
    ...
    if [ -f "10_setup_domiramacatops_poc.sh" ]; then
        ./10_setup_domiramacatops_poc.sh 2>&1 | tail -10
    fi
fi
echo ""
...
COUNT=$($CQLSH localhost 9042 -e "USE domiramacatops_poc; SELECT COUNT(*) FROM operations_by_account;" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]+" | head -1 | tr -d ' ' || echo "0")
...
START_TIME=$(date +%s.%N)
QUERY_OUTPUT=$($CQLSH localhost 9042 -e "USE domiramacatops_poc; $DEMO_QUERY;" 2>&1)
EXIT_CODE=$?
END_TIME=$(date +%s.%N)
...
# Statistiques globales
TOTAL=$($CQLSH localhost 9042 -e "USE domiramacatops_poc; SELECT COUNT(*) FROM operations_by_account;" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]+" | head -1 | tr -d ' ')
...
START_TIME=$(date +%s.%N)
QUERY_OUTPUT=$($CQLSH localhost 9042 -e "USE domiramacatops_poc; [QUERY];" 2>&1)
END_TIME=$(date +%s.%N)
QUERY_TIME=$(echo "$END_TIME - $START_TIME" | bc)
