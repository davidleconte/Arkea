...
INDEXES=$(./bin/cqlsh localhost 9042 -e "SELECT index_name FROM system_schema.indexes WHERE keyspace_name = 'domirama2_poc';" 2>&1 | grep -v "Warnings" | grep -v "index_name" | grep -vE "^---" | grep -v "^$" | wc -l | tr -d ' ')
...
if [ "$INDEXES" -ge 5 ]; then
    ...
    echo "   ┌─────────────────────────────────────────────────────────┐"
    ./bin/cqlsh localhost 9042 -e "SELECT index_name FROM system_schema.indexes WHERE keyspace_name = 'domirama2_poc';" 2>&1 | grep -v "Warnings" | grep -v "index_name" | grep -vE "^---" | grep -v "^$" | sed 's/^/   │ /'
    echo "   └─────────────────────────────────────────────────────────┘"
else
