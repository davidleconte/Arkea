...
def export_range(
    session, code_si: str, contrat: str, start_date: str, end_date: str, output_path: str
) -> Tuple[int, str]:
...
def test_migration_complexe():
    ...
    for start_date, end_date in test_ranges:
        ...
        # Export
        export_count, parquet_file = export_range(
            session, test_code_si, test_contrat, start_date, end_date, output_base
        )
    ...
    export_count1, parquet_file1 = export_range(
        session, test_code_si, test_contrat, start_date, end_date, output_base
    )
    export_count2, parquet_file2 = export_range(
        session, test_code_si, test_contrat, start_date, end_date, output_base
    )
    ...
    if loaded_checkpoint:
        print(
            f"✅ Checkpoint chargé : {len(loaded_checkpoint.get('completed_ranges', []))} plages complétées"
        )
    ...
    try:
        ...
        print("   ✅ Cohérence multi-tables validée")
    except Exception as e:
    ...
    print(
        f"✅ Cohérence globale : {'✅ Validée' if total_source == total_export else '❌ Incohérence'}"
    )
