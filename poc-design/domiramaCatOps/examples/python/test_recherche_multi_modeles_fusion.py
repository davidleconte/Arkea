...
def vector_search_byt5(session, query_embedding: List[float], code_si: str, contrat: str, limit: int = 5):
...
def vector_search_e5(session, query_embedding: List[float], code_si: str, contrat: str, limit: int = 5):
...
def vector_search_invoice(session, query_embedding: List[float], code_si: str, contrat: str, limit: int = 5):
...
def search_multi_models(session, query_text: str, code_si: str, contrat: str, limit: int = 5) -> Dict:
    ...
    try:
        ...
        results_invoice = vector_search_invoice(session, query_embedding_invoice, code_si, contrat, limit)
...
def test_multi_models_fusion():
    ...
    for query_text in test_queries:
        ...
        multi_results = search_multi_models(session, query_text, test_code_si, test_contrat, limit=5)
        search_time = time.time() - start_time
        
        print(f"   ⏱️  Temps de recherche : {search_time:.3f}s")
        print(f"   📊 Résultats par modèle :")
        ...
        print(f"   📋 Top 5 résultats fusionnés :")
        for idx, result in enumerate(merged_results[:5], 1):
            models_str = ", ".join(result['models'])
            print(f"      {idx}. {result['libelle']} (modèles: {models_str}, score: {result['best_score']:.2f})")
    ...
    try:
        ...
        try:
            ...
            results = vector_search_invoice(session, query_embedding, test_code_si, test_contrat, limit=5)
    ...
    for summary in results_summary:
        print(f"✅ '{summary['query']}' : {summary['merged_count']} résultats fusionnés en {summary['search_time']:.3f}s")
    ...
    return {
        'success': len(results_summary) == len(test_queries),
