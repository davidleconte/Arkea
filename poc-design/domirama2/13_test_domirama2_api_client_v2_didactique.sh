...
BEFORE_STATE=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT code_si, contrat, date_op, numero_op, libelle, cat_auto, cat_user, cat_date_user, cat_validee FROM operations_by_account WHERE code_si = '01' AND contrat = '1234567890' ORDER BY date_op DESC LIMIT 10;" 2>&1 | grep -v "Warnings" | head -15)

# Exécuter les tests et capturer la sortie
info "   Exécution en cours..."
API_OUTPUT=$(timeout 30 ./bin/cqlsh localhost 9042 -f "$API_FILE" 2>&1) || API_EXIT_CODE=$?
...
AFTER_STATE=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT code_si, contrat, date_op, numero_op, libelle, cat_auto, cat_user, cat_date_user, cat_validee FROM operations_by_account WHERE code_si = '01' AND contrat = '1234567890' ORDER BY date_op DESC LIMIT 10;" 2>&1 | grep -v "Warnings" | head -15)
...
CORRECTED_SAMPLE=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT cat_user FROM operations_by_account WHERE code_si = '01' AND contrat = '1234567890' LIMIT 10;" 2>&1 | grep -v "Warnings" | grep -v "cat_user" | grep -vE "^---" | grep -v "^$" | grep -v "null" | head -1)
...
AUTO_SAMPLE=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT cat_auto FROM operations_by_account WHERE code_si = '01' AND contrat = '1234567890' LIMIT 10;" 2>&1 | grep -v "Warnings" | grep -v "cat_auto" | grep -vE "^---" | grep -v "^$" | grep -v "null" | head -1)
...
echo "   ┌─────────────────────────────────────────────────────────┐"
./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT date_op, numero_op, libelle, cat_auto, cat_user, cat_date_user, cat_validee FROM operations_by_account WHERE code_si = '01' AND contrat = '1234567890' ORDER BY date_op DESC LIMIT 5;" 2>&1 | grep -v "Warnings" | head -10 | sed 's/^/   │ /'
echo "   └─────────────────────────────────────────────────────────┘"
echo ""
