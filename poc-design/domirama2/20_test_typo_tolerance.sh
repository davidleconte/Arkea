...
echo ""
demo "📊 Résultats :"
./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT libelle, montant FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle_prefix : 'loyer' LIMIT 5;" 2>&1 | grep -v "Warnings" | head -10
echo ""
...
./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT libelle, montant FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle : 'loyers' LIMIT 3;" 2>&1 | grep -v "Warnings" | head -8
echo ""
echo "Avec libelle_prefix (sans stemming 'loyers') :"
./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT libelle, montant FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle_prefix : 'loyers' LIMIT 3;" 2>&1 | grep -v "Warnings" | head -8
echo ""
...
./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT COUNT(*) FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle : 'loyr' ALLOW FILTERING;" 2>&1 | grep -v "Warnings" | head -3
echo ""
echo "Avec libelle_prefix (préfixe 'loy') :"
./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT COUNT(*) FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle_prefix : 'loy' ALLOW FILTERING;" 2>&1 | grep -v "Warnings" | head -3
echo ""
