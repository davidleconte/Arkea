...
def test_aggregations_combinees(session, code_si: str, contrat: str):
    ...
    try:
        ...
        for row in rows:
            if row.date_op and row.cat_auto:
                ...
                if key not in combined_stats:
                    combined_stats[key] = {'count': 0, 'sum': 0.0, 'day': day_key, 'category': row.cat_auto}
        ...
        print(f"   📊 Statistiques combinées (date + catégorie) :")
        for key, stats in sorted(combined_stats.items())[:10]:  # Afficher les 10 premiers
            print(f"      {stats['day']} - {stats['category']} : COUNT={stats['count']}, SUM={stats['sum']:.2f}")
        
        return {
            'combined_stats': combined_stats,
            'total_combinations': len(combined_stats)
        }
    except Exception as e:
        print(f"   ⚠️  Erreur : {e}")
        return None
...
def test_performance_aggregations(session, code_si: str, contrat: str):
    ...
    for limit in limits:
        ...
        print(f"   Limite {limit} : {len(rows)} lignes, {len(category_stats)} catégories, {elapsed:.3f}s")
...
def test_aggregations():
    ...
    if 'categorie' in results:
        print(f"✅ Agrégations par catégorie : {results['categorie']['total_categories']} catégories")
    if 'combinees' in results:
        print(f"✅ Agrégations combinées : {results['combinees']['total_combinations']} combinaisons")
