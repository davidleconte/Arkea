...
PYTHON_SCRIPT="${PYTHON_DIR}/test_fenetre_glissante_complexe.py"
REPORT_FILE="${SCRIPT_DIR}/../doc/demonstrations/21_FENETRE_GLISSANTE_COMPLEXE_DEMONSTRATION.md"

show_demo_header "Test Complexe P2-01 : Fenêtre Glissante Complexe"

info "📚 Ce test valide :"
echo "   ✅ Fenêtre glissante avec chevauchement"
echo "   ✅ Validation cohérence entre fenêtres"
echo "   ✅ Gestion des frontières (début/fin de période)"
echo "   ✅ Agrégation multi-fenêtres"

echo ""
info "🚀 Exécution du test..."

if python3 "${PYTHON_SCRIPT}" 2>&1 | tee /tmp/test_fenetre_glissante_complexe_output.txt; then
...
output_file = '/tmp/test_fenetre_glissante_complexe_output.txt'

report = f"""# 🔍 Test Complexe P2-01 : Fenêtre Glissante Complexe
