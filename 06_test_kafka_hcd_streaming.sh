...
if ./kafka-helper.sh kafka-topics.sh --list --bootstrap-server localhost:9092 2>&1 | grep -q "test-topic"; then
    info "✅ Topic test-topic existe"
else
    warn "⚠️  Création du topic test-topic..."
    ./kafka-helper.sh kafka-topics.sh --create --bootstrap-server localhost:9092 --topic test-topic --partitions 1 --replication-factor 1 2>&1 | grep -v "Error" || true
    sleep 2
fi
...
echo "Message test 1" | ./kafka-helper.sh kafka-console-producer.sh --bootstrap-server localhost:9092 --topic test-topic 2>&1 | grep -v "Warnings" || true
sleep 1
echo "Message test 2" | ./kafka-helper.sh kafka-console-producer.sh --bootstrap-server localhost:9092 --topic test-topic 2>&1 | grep -v "Warnings" || true
sleep 1
echo '{"user": "alice", "action": "login", "timestamp": "2025-11-25T13:00:00Z"}' | ./kafka-helper.sh kafka-console-producer.sh --bootstrap-server localhost:9092 --topic test-topic 2>&1 | grep -v "Warnings" || true
sleep 1
echo '{"user": "bob", "action": "logout", "timestamp": "2025-11-25T13:01:00Z"}' | ./kafka-helper.sh kafka-console-producer.sh --bootstrap-server localhost:9092 --topic test-topic 2>&1 | grep -v "Warnings" || true
