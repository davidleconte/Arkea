...
if ! $CQLSH localhost 9042 -e "DESCRIBE KEYSPACE domirama2_poc;" > /dev/null 2>&1; then
...
COLUMN_EXISTS=$($CQLSH localhost 9042 -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -c "libelle_prefix" || echo "0")
...
# Exécuter la requête
START_TIME=$(date +%s.%N)
QUERY_OUTPUT=$($CQLSH localhost 9042 -e "USE domirama2_poc; SELECT libelle, montant FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle_prefix : 'loyer' LIMIT 5;" 2>&1)
EXIT_CODE=$?
END_TIME=$(date +%s.%N)
...
START_TIME=$(date +%s.%N)
QUERY_OUTPUT_LIBELLE=$($CQLSH localhost 9042 -e "USE domirama2_poc; SELECT libelle, montant FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle : 'loyers' LIMIT 3;" 2>&1)
EXIT_CODE_LIBELLE=$?
END_TIME=$(date +%s.%N)
...
START_TIME=$(date +%s.%N)
QUERY_OUTPUT_PREFIX=$($CQLSH localhost 9042 -e "USE domirama2_poc; SELECT libelle, montant FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle_prefix : 'loyers' LIMIT 3;" 2>&1)
EXIT_CODE_PREFIX=$?
END_TIME=$(date +%s.%N)
...
START_TIME=$(date +%s.%N)
QUERY_OUTPUT_TYPO_LIBELLE=$($CQLSH localhost 9042 -e "USE domirama2_poc; SELECT COUNT(*) FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle : 'loyr' ALLOW FILTERING;" 2>&1)
...
START_TIME=$(date +%s.%N)
QUERY_OUTPUT_TYPO_PREFIX=$($CQLSH localhost 9042 -e "USE domirama2_poc; SELECT COUNT(*) FROM operations_by_account WHERE code_si = '$CODE_SI' AND contrat = '$CONTRAT' AND libelle_prefix : 'loy' ALLOW FILTERING;" 2>&1)
