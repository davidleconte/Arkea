...
FIRST_ACCOUNT=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "USE domirama2_poc; SELECT code_si, contrat FROM operations_by_account WHERE libelle : 'loyer' AND libelle : 'paris' LIMIT 1;" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]" | head -1)
...
TEST_OUTPUT=$(timeout 60 ./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -f "$TEMP_CQL" 2>&1) || TEST_EXIT_CODE=$?
...
RESULT1=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "$CQL_QUERY1" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]+" | head -1 | tr -d " ")
...
RESULT2=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "$CQL_QUERY2" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]+" | head -1 | tr -d " ")
...
RESULT11=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "$CQL_QUERY11" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]+" | head -1 | tr -d " ")
...
RESULT18=$(./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "$CQL_QUERY18" 2>&1 | grep -v "Warnings" | grep -E "^[[:space:]]*[0-9]+" | head -1 | tr -d " ")
if [ -n "$RESULT18" ]; then
