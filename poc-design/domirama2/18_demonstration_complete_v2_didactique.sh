...
jenv local 11
eval "$(jenv init -)"
$CQLSH localhost 9042 -f "${SCRIPT_DIR}/schemas/06_create_libelle_tokens_collection.cql" 2>&1 | grep -v "Warnings" | tail -5
...
if [ -f "${SCRIPT_DIR}/schemas/03_create_domirama2_schema_fuzzy.cql" ]; then
    ...
    $CQLSH localhost 9042 -f "${SCRIPT_DIR}/schemas/03_create_domirama2_schema_fuzzy.cql" 2>&1 | grep -v "Warnings" | tail -5
...
if [ -f "${SCRIPT_DIR}/scripts/add_missing_test_data.cql" ]; then
    $CQLSH localhost 9042 -f "${SCRIPT_DIR}/scripts/add_missing_test_data.cql" 2>&1 | grep -v "Warnings" | tail -10
...
execute_demo() {
    ...
    FULL_QUERY="USE domirama2_poc; $DEMO_QUERY;"
    QUERY_OUTPUT=$($CQLSH localhost 9042 -e "$FULL_QUERY" 2>&1) || true
...
2. [Types de Recherches Avancées](#types-de-recherches-avancées)
3. [Cas d'Usage](#cas-dusage)
4. [Orchestration](#orchestration)
5. [Détails des {len(demos)} Démonstrations](#détails-des-démonstrations)
6. [Résumé des Résultats](#résumé-des-résultats)
