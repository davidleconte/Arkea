# 📑 Index de la Documentation - Domirama2

**Date** : 2025-01-XX
**Structure** : Organisation par catégories

---

## 📁 Structure des Catégories

### 🎨 Design et Architecture

Documents de design, analyse, architecture et data model.

**Répertoire** : [`design/`](design/)

**Fichiers principaux** :

- `02_VALUE_PROPOSITION_DOMIRAMA2.md` - Proposition de valeur
- `03_GAPS_ANALYSIS.md` - Analyse des gaps fonctionnels
- `04_BILAN_ECARTS_FONCTIONNELS.md` - Bilan des écarts fonctionnels
- `05_AUDIT_COMPLET_GAP_FONCTIONNEL.md` - Audit complet des gaps
- `24_PARQUET_VS_ORC_ANALYSIS.md` - Analyse Parquet vs ORC
- `43_SYNTHESE_COMPLETE_ANALYSE_2024.md` - Synthèse complète d'analyse

[Voir tous les fichiers design →](design/)

---

### 📖 Guides et Références

Guides d'utilisation, README par fonctionnalité et références.

**Répertoire** : [`guides/`](guides/)

**Fichiers principaux** :

- `01_README.md` - Vue d'ensemble du POC Domirama2
- `06_README_INDEX_AVANCES.md` - Index SAI avancés
- `07_README_FUZZY_SEARCH.md` - Recherche floue (Vector search)
- `08_README_HYBRID_SEARCH.md` - Recherche hybride (Full-Text + Vector)
- `09_README_MULTI_VERSION.md` - Multi-version / Time travel
- `11_README_EXPORT_INCREMENTAL.md` - Exports incrémentaux
- `18_README_DATA_API.md` - Data API

[Voir tous les guides →](guides/)

---

### 🔧 Implémentations

Documents d'implémentation et de développement.

**Répertoire** : [`implementation/`](implementation/)

**Fichiers principaux** :

- `10_TIME_TRAVEL_IMPLEMENTATION.md` - Implémentation time travel
- `20_IMPLEMENTATION_OFFICIELLE_DATA_API.md` - Implémentation Data API officielle
- `21_STATUT_DATA_API.md` - Statut Data API
- `32_CONFORMITE_DATA_API_HCD.md` - Conformité Data API HCD

[Voir toutes les implémentations →](implementation/)

---

### 📊 Résultats

Résultats de tests, validations et démonstrations.

**Répertoire** : [`results/`](results/)

**Fichiers principaux** :

- `22_DEMONSTRATION_RESUME.md` - Résumé des démonstrations
- `23_DEMONSTRATION_VALIDATION.md` - Validation des démonstrations
- `42_DEMONSTRATION_COMPLETE_DOMIRAMA.md` - Démonstration complète

[Voir tous les résultats →](results/)

---

### 🔧 Corrections

Corrections appliquées et améliorations.

**Répertoire** : [`corrections/`](corrections/)

**Fichiers principaux** :

- `44_GUIDE_AMELIORATION_SCRIPTS.md` - Guide d'amélioration des scripts
- `45_GUIDE_GENERALISATION_CAPTURE_RESULTATS.md` - Généralisation capture résultats
- `69_AMELIORATION_SCRIPTS_16_17_18.md` - Améliorations scripts 16-18

[Voir toutes les corrections →](corrections/)

---

### 🔍 Audits

Audits et analyses complètes.

**Répertoire** : [`audits/`](audits/)

**Fichiers principaux** :

- `AUDIT_COMPLET_2025.md` - Audit complet du répertoire (2025)
- `AUDIT_SCRIPTS_SHELL_2025.md` - Audit scripts shell (2025)
- `36_STANDARDS_SCRIPTS_SHELL.md` - Standards scripts shell
- `37_AUDIT_DOCUMENTATION_SCRIPTS.md` - Audit documentation scripts

[Voir tous les audits →](audits/)

---

### 🎬 Démonstrations

Rapports auto-générés des démonstrations.

**Répertoire** : [`demonstrations/`](demonstrations/)

**Démonstrations récentes** :

- `40_LIKE_PATTERNS_DEMO.md` - Démonstration des patterns LIKE de base
- `41_WILDCARD_SEARCH_DEMO.md` - Démonstration de la recherche wildcard avancée (multi-champs, filtres combinés)

[Voir toutes les démonstrations →](demonstrations/)

---

### 📝 Templates

Templates réutilisables pour scripts didactiques.

**Répertoire** : [`templates/`](templates/)

[Voir tous les templates →](templates/)

---

## 🔍 Navigation Rapide

### Par Type de Document

- **Commencer par** : [`guides/01_README.md`](guides/01_README.md)
- **Design** : [`design/02_VALUE_PROPOSITION_DOMIRAMA2.md`](design/02_VALUE_PROPOSITION_DOMIRAMA2.md)
- **Guides** : [`guides/06_README_INDEX_AVANCES.md`](guides/06_README_INDEX_AVANCES.md)
- **Résultats** : [`results/22_DEMONSTRATION_RESUME.md`](results/22_DEMONSTRATION_RESUME.md)
- **Audits** : [`audits/AUDIT_COMPLET_2025.md`](audits/AUDIT_COMPLET_2025.md)

### Par Parcours de Lecture

#### Pour Comprendre le POC (Débutant)

1. [`guides/01_README.md`](guides/01_README.md) - Vue d'ensemble
2. [`design/02_VALUE_PROPOSITION_DOMIRAMA2.md`](design/02_VALUE_PROPOSITION_DOMIRAMA2.md) - Proposition de valeur
3. [`design/03_GAPS_ANALYSIS.md`](design/03_GAPS_ANALYSIS.md) - Analyse des gaps

#### Pour Implémenter une Fonctionnalité

- **Recherche** : [`guides/06_README_INDEX_AVANCES.md`](guides/06_README_INDEX_AVANCES.md), [`guides/07_README_FUZZY_SEARCH.md`](guides/07_README_FUZZY_SEARCH.md), [`guides/08_README_HYBRID_SEARCH.md`](guides/08_README_HYBRID_SEARCH.md)
- **Multi-version** : [`guides/09_README_MULTI_VERSION.md`](guides/09_README_MULTI_VERSION.md), [`implementation/10_TIME_TRAVEL_IMPLEMENTATION.md`](implementation/10_TIME_TRAVEL_IMPLEMENTATION.md)
- **Exports** : [`guides/11_README_EXPORT_INCREMENTAL.md`](guides/11_README_EXPORT_INCREMENTAL.md), [`guides/12_README_EXPORT_SPARK_SUBMIT.md`](guides/12_README_EXPORT_SPARK_SUBMIT.md)
- **Data API** : [`guides/18_README_DATA_API.md`](guides/18_README_DATA_API.md), [`implementation/20_IMPLEMENTATION_OFFICIELLE_DATA_API.md`](implementation/20_IMPLEMENTATION_OFFICIELLE_DATA_API.md)

#### Pour Auditer/Valider

- [`design/04_BILAN_ECARTS_FONCTIONNELS.md`](design/04_BILAN_ECARTS_FONCTIONNELS.md) - Bilan des écarts
- [`design/05_AUDIT_COMPLET_GAP_FONCTIONNEL.md`](design/05_AUDIT_COMPLET_GAP_FONCTIONNEL.md) - Audit gaps
- [`results/22_DEMONSTRATION_RESUME.md`](results/22_DEMONSTRATION_RESUME.md) - Résumé démonstrations
- [`audits/AUDIT_COMPLET_2025.md`](audits/AUDIT_COMPLET_2025.md) - Audit complet

### Par Numéro

Les fichiers sont toujours numérotés (00_, 01_, etc.) pour préserver l'ordre chronologique dans chaque catégorie.

---

## 📚 Documentation à la Racine

- [`00_ORGANISATION_DOC.md`](00_ORGANISATION_DOC.md) - Guide de lecture de la documentation
- [`ANALYSE_STRUCTURE_DOMIRAMACATOPS.md`](ANALYSE_STRUCTURE_DOMIRAMACATOPS.md) - Analyse de la structure domiramaCatOps
- [`PLAN_REORGANISATION_STRUCTURE.md`](PLAN_REORGANISATION_STRUCTURE.md) - Plan de réorganisation

---

**Date de création** : 2025-01-XX
**Version** : 1.0
