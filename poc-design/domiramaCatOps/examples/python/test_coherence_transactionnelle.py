...
def test_referential_coherence(session, code_si: str, contrat: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if ops_count > 0:
            print(f"   ✅ Cohérence référentielle : operations_by_account existe ({ops_count} opérations)")
            return True, f"✅ {ops_count} opérations trouvées"
        else:
            print(f"   ⚠️  Aucune opération trouvée pour cette partition")
...
def test_historical_coherence(session, code_si: str, contrat: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if opp_count > 0:
            print(f"   ✅ Cohérence historique : historique_opposition → opposition_categorisation")
            return True, f"✅ PSE {no_pse} trouvé dans opposition_categorisation"
        else:
            print(f"   ⚠️  PSE {no_pse} non trouvé dans opposition_categorisation (peut être normal)")
...
def test_rules_coherence(session) -> Tuple[bool, str]:
    ...
    # Note: HCD ne supporte pas IS NOT NULL ni SELECT DISTINCT, on récupère toutes les catégories et on filtre côté client
    query_ops = f"""
    SELECT cat_auto
    FROM {KEYSPACE}.operations_by_account
    LIMIT 100
    ...
    try:
        ...
        if not missing_cats:
            print(f"   ✅ Cohérence règles : toutes les catégories trouvées dans regles_personnalisees")
