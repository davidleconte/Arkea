...
if [ -f "$SCRIPT_DIR/10_setup_domirama2_poc.sh" ]; then
    ...
    SCHEMA_CHECK=$(cd "$HCD_DIR" && ./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -c "idx_libelle_fulltext" || echo "0")
...
if [ -f "$SCRIPT_DIR/21_setup_fuzzy_search.sh" ]; then
    ...
    cd "$SCRIPT_DIR"
    SCHEMA_CHECK=$(cd "$HCD_DIR" && ./bin/cqlsh "$HCD_HOST" "$HCD_PORT" -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -c "libelle_embedding" || echo "0")
    if [ "$SCHEMA_CHECK" -eq 0 ]; then
