...
def concurrent_read_test(session, code_si: str, contrat: str, num_threads: int = 10, queries_per_thread: int = 10):
    """Test de charge lecture concurrente"""
    print(f"📋 TEST 1 : Charge Lecture Concurrente ({num_threads} threads, {queries_per_thread} requêtes/thread)")
    print("-" * 70)
...
def concurrent_write_test(session, code_si: str, contrat: str, num_threads: int = 10, inserts_per_thread: int = 10):
    ...
    if all_latencies:
        ...
        print(f"   ❌ Aucune insertion réussie")
        return None

def mixed_workload_test(session, code_si: str, contrat: str, num_threads: int = 10, ops_per_thread: int = 10):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        # 50% threads écriture
        write_futures = [executor.submit(write_worker, i) for i in range(num_threads // 2, num_threads)]
...
def test_charge_concurrente():
    ...
    result1 = concurrent_read_test(session, test_code_si, test_contrat, num_threads=10, queries_per_thread=10)
    if result1:
        results.append(result1)
    
    # Test 2 : Charge écriture
    result2 = concurrent_write_test(session, test_code_si, test_contrat, num_threads=10, inserts_per_thread=10)
    if result2:
        results.append(result2)
    
    # Test 3 : Charge mixte
    result3 = mixed_workload_test(session, test_code_si, test_contrat, num_threads=10, ops_per_thread=5)
    if result3:
    ...
    for result in results:
        if result['type'] == 'read':
            print(f"✅ Lecture : {result['throughput']:.2f} req/s, p95={result['latencies']['p95']:.2f}ms")
        elif result['type'] == 'write':
            print(f"✅ Écriture : {result['throughput']:.2f} inserts/s, p95={result['latencies']['p95']:.2f}ms")
