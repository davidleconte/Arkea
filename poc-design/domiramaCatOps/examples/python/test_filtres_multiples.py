...
def test_cas_limites_filtres(session, code_si: str, contrat: str):
    ...
    print(
        "      Résultats : "
        f"{len(results1)} ({'✅ Aucun résultat' if len(results1) == 0 else '⚠️  Résultats inattendus'})"
    )
    ...
    print(
        "      Résultats : "
        f"{len(results2)} ({'✅ Nombre raisonnable' if len(results2) <= 100 else '⚠️  Trop de résultats'})"
    )
...
def test_filtres_multiples():
    ...
    if "multiple_filters" in results:
        ...
        print(
            ...
            f"{'✅ Oui' if results['multiple_filters']['all_filters_valid'] else '⚠️  Non'}"
        )
