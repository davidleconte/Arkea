...
def test_performance_facettes(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        start = time.time()
        session.execute(
            f"SELECT cat_auto FROM {KEYSPACE}.operations_by_account "
            f"WHERE code_si = '{code_si}' AND contrat = '{contrat}'"
        )
        ...
        start = time.time()
        session.execute(
            f"SELECT date_op FROM {KEYSPACE}.operations_by_account "
            f"WHERE code_si = '{code_si}' AND contrat = '{contrat}'"
        )
        ...
        start = time.time()
        session.execute(
            f"SELECT cat_auto, date_op FROM {KEYSPACE}.operations_by_account "
            f"WHERE code_si = '{code_si}' AND contrat = '{contrat}'"
        )
