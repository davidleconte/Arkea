...
def migrate_script(script_path):
        ...
        if re.search(pattern, content):
            ...
            for i, line in enumerate(lines):
                ...
                if skip_until_script_dir:
                    if line.strip().startswith('SCRIPT_DIR=') or line.strip().startswith('SCHEMA_FILE=') or line.strip().startswith('PARQUET_FILE=') or line.strip().startswith('RED='):
                        skip_until_script_dir = False
    ...
    replacements = [
        ...
        (r'"localhost" 9042', '"$HCD_HOST" "$HCD_PORT"'),
        (r"'localhost' 9042", '"$HCD_HOST" "$HCD_PORT"'),
        (r'--conf spark\.cassandra\.connection\.host=localhost', '--conf spark.cassandra.connection.host="$HCD_HOST"'),
        (r'--conf spark\.cassandra\.connection\.port=9042', '--conf spark.cassandra.connection.port="$HCD_PORT"'),
        (r'\.config\("spark\.cassandra\.connection\.host", "localhost"\)', '.config("spark.cassandra.connection.host", "$HCD_HOST")'),
        (r'\.config\("spark\.cassandra\.connection\.port", "9042"\)', '.config("spark.cassandra.connection.port", "$HCD_PORT")'),
    ]
