...
def main():
    ...
    for test_case in test_cases:
        ...
        if ann_rank_in_top5 is not None:
            ...
            print("   → La recherche ANN fonctionne, mais le résultat n'était peut-être pas le premier")
        elif ann_rank_in_top20 is not None:
            print(f"⚠️  **Résultat pertinent trouvé mais hors top 5** : Rang {ann_rank_in_top20}")
            print("   → Cause : La similarité cosinus n'est pas assez élevée pour être dans le top 5")
        ...
        if similarities:
            ...
            if ann_results:
                ...
                if ann_similarities:
                    ...
                    print(f"📊 Similarité maximale dans résultats ANN (top 5) : {top_ann_similarity:.4f}")
                    print()
                    
                    if top_similarity > top_ann_similarity:
                        print("⚠️  **Problème identifié** :")
                        print(f"   - La similarité avec les libellés pertinents ({top_similarity:.4f}) est PLUS ÉLEVÉE")
                        print(f"   - Que la similarité avec les résultats ANN ({top_ann_similarity:.4f})")
                        print("   → Cause : Les libellés pertinents ne sont pas indexés correctement ou")
