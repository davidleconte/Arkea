...
# =============================================================================
# Script : Correction Automatique des Références localhost Hardcodées
# =============================================================================
# Date : 2025-12-02
# Usage : ./96_fix_localhost_references.sh [--dry-run] [--file FILE] [--help]
# Description : Remplace les références localhost hardcodées par variables d'environnement
# =============================================================================
...
DRY_RUN=false
TARGET_FILE=""
BACKUP_DIR="${ARKEA_HOME}/.backup_localhost_$(date +%Y%m%d_%H%M%S)"

# Patterns de localhost hardcodés à corriger
declare -A LOCALHOST_PATTERNS=(
    # HCD/Cassandra
    ...
    # Références simples localhost (dans contexte de connexion)
    ["HCD est démarré sur localhost:9042"]="HCD est démarré sur \${HCD_HOST:-localhost}:\${HCD_PORT:-9042}"
    ["cassandra sur localhost:9042"]="cassandra sur \${HCD_HOST:-localhost}:\${HCD_PORT:-9042}"
)
...
print_help() {
    echo "Usage: ./96_fix_localhost_references.sh [OPTIONS]"
    echo ""
    echo "Options:"
    ...
    echo ""
    echo "Exemples:"
    echo "  ./96_fix_localhost_references.sh --dry-run"
    echo "  ./96_fix_localhost_references.sh --file scripts/setup/01_install_hcd.sh"
    echo "  ./96_fix_localhost_references.sh"
}
...
fix_file() {
    ...
    # Vérifier si le fichier contient des références localhost hardcodées
    ...
    # Appliquer les corrections (ordre important : plus spécifique d'abord)
    for pattern in "${!LOCALHOST_PATTERNS[@]}"; do
        local replacement="${LOCALHOST_PATTERNS[$pattern]}"
...
find_files_to_fix() {
    ...
    if [ -n "$TARGET_FILE" ]; then
        ...
        # Rechercher tous les fichiers avec références localhost hardcodées
        while IFS= read -r file; do
            ...
            ! -path "*/archive/*" \
            \( -name "*.sh" -o -name "*.scala" -o -name "*.py" \) 2>/dev/null)
    fi
...
section "Correction Automatique des Références localhost Hardcodées - ARKEA"
...
if [ ${#files_to_fix[@]} -eq 0 ]; then
    info "Aucun fichier avec références localhost hardcodées détecté"
    exit 0
fi
