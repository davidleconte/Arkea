...
def main():
    ...
    for query1, query2, description in test_cases:
        ...
        print("   📊 Similarité entre les deux requêtes : {similarity:.3f}")
        if similarity >= 0.6:
        ...
        if not relevance1["is_mostly_relevant"] or not relevance2["is_mostly_relevant"]:
            print("   ⚠️  Pertinence faible : Les résultats peuvent ne pas être pertinents")
            print(
                f"   💡 Note : Vérifiez que les données de test contiennent des libellés pertinents"
            )
