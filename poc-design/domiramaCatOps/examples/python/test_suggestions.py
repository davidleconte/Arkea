...
def test_suggestions_by_prefix(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        for prefix in prefixes:
            # Recherche full-text avec préfixe (sans DISTINCT, déduplication côté client)
            query = f"""
            SELECT libelle
            ...
            suggestions = list(set([row.libelle for row in result if row.libelle and row.libelle.upper().startswith(prefix.upper())]))[:10]
        ...
        return True, f"Suggestions par préfixe validées", suggestions_by_prefix
    except Exception as e:
...
def test_autocompletion(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        for partial in partial_queries:
            ...
            # Note: CQL ne supporte pas LIKE, on utilise full-text search (sans DISTINCT)
            query = f"""
            SELECT libelle
            FROM {KEYSPACE}.operations_by_account
            WHERE code_si = '{code_si}' AND contrat = '{contrat}'
            ...
            result = session.execute(query)
            # Déduplication côté client
            matches = list(set([row.libelle for row in result if row.libelle and partial.upper() in row.libelle.upper()]))[:5]
            latency = (time.time() - start) * 1000
            ...
            print(f"   ✅ '{partial}' : {len(matches)} correspondances, {len(completions_list)} complétions en {latency:.2f}ms")
        
        return True, f"Autocomplétion validée", completions
    except Exception as e:
        return False, f"Erreur : {e}", {}
...
def test_suggestions_with_scoring(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        for row in result:
            if row.libelle:
                # Calculer un score de pertinence basique (basé sur la position dans les résultats)
        ...
        return True, f"Suggestions avec scoring validées ({len(suggestions)} suggestions)", scoring_info
    except Exception as e:
        return False, f"Erreur : {e}", {}
...
def test_suggestions_performance(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        print(f"   ✅ Latence moyenne : {avg_latency:.2f}ms (min: {min_latency:.2f}ms, max: {max_latency:.2f}ms)")
        if avg_latency < 50:
            print(f"   ✅ Cible < 50ms atteinte")
        else:
            print(f"   ⚠️  Cible < 50ms non atteinte")
...
def test_suggestions_relevance(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        for prefix, expected_keywords in test_cases:
            ...
            # Calculer la pertinence (nombre de suggestions contenant les mots-clés attendus)
            relevant_count = sum(1 for sug in suggestions if any(kw.upper() in sug.upper() for kw in expected_keywords))
            ...
            print(f"   ✅ Préfixe '{prefix}' : {relevance_score:.1f}% de pertinence ({relevant_count}/{len(suggestions)})")
