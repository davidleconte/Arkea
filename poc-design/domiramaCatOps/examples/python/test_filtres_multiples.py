...
def test_vector_fulltext_date_montant_categorie(session, code_si: str, contrat: str):
    ...
    if all_valid:
        print("   ✅ Tous les filtres respectés")
    else:
        print("   ⚠️  Certains filtres non respectés")
...
def test_optimisation_ordre_filtres(session, code_si: str, contrat: str):
    ...
    results1, time1 = search_with_multiple_filters(
        session, query_text, code_si, contrat, date_start=date_start, date_end=date_end, limit=5
    )
    ...
    results2, time2 = search_with_multiple_filters(session, query_text, code_si, contrat, limit=5)
    ...
    if time1 < time2:
        ...
        print("   ⚠️  Filtre sélectif n'améliore pas la performance")
...
def test_performance_filtres_multiples(session, code_si: str, contrat: str):
    ...
    for name, filters in test_cases:
        ...
        results[name] = {"count": len(search_results), "time": elapsed}
...
def test_cas_limites_filtres(session, code_si: str, contrat: str):
    ...
    results1, time1 = search_with_multiple_filters(
        ...
        date_end="2020-01-02",
        montant_min=1000000.0,
        montant_max=2000000.0,
        categorie="INEXISTANTE",
        limit=5,
    )
    print(
        f"      Résultats : {len(results1)} ({'✅ Aucun résultat' if len(results1) == 0 else '⚠️  Résultats inattendus'})"
    )

    # Cas 2 : Trop de résultats (filtres peu restrictifs)
    print("   Cas 2 : Filtres peu restrictifs (beaucoup de résultats)")
    results2, time2 = search_with_multiple_filters(session, "LOYER", code_si, contrat, limit=100)
    print(
        f"      Résultats : {len(results2)} ({'✅ Nombre raisonnable' if len(results2) <= 100 else '⚠️  Trop de résultats'})"
    )

    return {"no_results": len(results1), "many_results": len(results2)}
...
def test_filtres_multiples():
    ...
    if "multiple_filters" in results:
        ...
        print(
            f"   Tous les filtres respectés : {'✅ Oui' if results['multiple_filters']['all_filters_valid'] else '⚠️  Non'}"
        )
