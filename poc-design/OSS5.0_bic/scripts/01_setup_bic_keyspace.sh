...
# Configuration cqlsh
# OSS5.0: Use Podman container or binary based on HCD_DIR
if [ "$HCD_DIR" = "podman" ] || [ -z "$HCD_DIR" ]; then
    # Podman mode: use podman exec
    CQLSH="podman exec arkea-hcd cqlsh localhost 9042"
else
    # Binary mode: use local cqlsh
...
# Vérifier que HCD est disponible (Podman or binary)
if [ "$HCD_DIR" = "podman" ] || [ -z "$HCD_DIR" ]; then
    # Check Podman container
    if ! podman ps --filter "name=arkea-hcd" --format "{{.Names}}" 2>/dev/null | grep -q "arkea-hcd"; then
        echo -e "${RED}❌ Erreur: Container arkea-hcd n'est pas en cours d'exécution${NC}"
        echo "   Exécutez d'abord: make demo"
        exit 1
    fi
...
# Pour Podman: copier le schéma dans le container
if [ "$HCD_DIR" = "podman" ] || [ -z "$HCD_DIR" ]; then
    podman cp "$SCHEMA_FILE" arkea-hcd:/tmp/$(basename "$SCHEMA_FILE")
    CQLSH_EXEC="podman exec arkea-hcd cqlsh localhost 9042 -f /tmp/$(basename "$SCHEMA_FILE")"
else
    CQLSH_EXEC="$CQLSH -f $SCHEMA_FILE"
fi
...
if [ -f "$SCHEMA_FILE" ]; then
    if [ "$HCD_DIR" = "podman" ] || [ -z "$HCD_DIR" ]; then
        # Podman mode: copy schema to container and execute
        podman cp "$SCHEMA_FILE" arkea-hcd:/tmp/$(basename "$SCHEMA_FILE")
