...
KEYSpace_CHECK=$(./bin/cqlsh localhost 9042 -e "DESCRIBE KEYSPACE domiramacatops_poc;" 2>&1 | grep -v "Warnings" | head -20)
...
TABLE_CHECK=$(./bin/cqlsh localhost 9042 -e "DESCRIBE TABLE domiramacatops_poc.operations_by_account;" 2>&1 | grep -v "Warnings")
...
INDEXES=$(./bin/cqlsh localhost 9042 -e "SELECT index_name FROM system_schema.indexes WHERE keyspace_name = 'domiramacatops_poc';" 2>&1 | grep -v "Warnings" | grep -v "index_name" | grep -v "---" | grep -v "^$" | wc -l | tr -d ' ')
...
if [ "$INDEXES" -ge 5 ]; then
    ...
    echo "   ┌─────────────────────────────────────────────────────────┐"
    ./bin/cqlsh localhost 9042 -e "SELECT index_name FROM system_schema.indexes WHERE keyspace_name = 'domiramacatops_poc';" 2>&1 | grep -v "Warnings" | grep -v "index_name" | grep -v "---" | grep -v "^$" | sed 's/^/   │ /'
    echo "   └─────────────────────────────────────────────────────────┘"
else
...
TABLE_EXISTS=$(./bin/cqlsh localhost 9042 -e "SELECT table_name FROM system_schema.tables WHERE keyspace_name = 'domiramacatops_poc' AND table_name = 'operations_by_account';" 2>&1 | grep -v "Warnings" | grep -c "operations_by_account" || echo "0")
...
COLUMNS=$(./bin/cqlsh localhost 9042 -e "DESCRIBE TABLE domiramacatops_poc.operations_by_account;" 2>&1 | grep -E "(cat_auto|cat_confidence|cat_user|cat_date_user|cat_validée)" | wc -l | tr -d ' ')
if [ "$COLUMNS" -ge 5 ]; then
    success "✅ Toutes les colonnes de catégorisation présentes ($COLUMNS/5)"
    echo ""
    result "📊 Colonnes de catégorisation trouvées :"
    echo "   ┌─────────────────────────────────────────────────────────┐"
    ./bin/cqlsh localhost 9042 -e "DESCRIBE TABLE domiramacatops_poc.operations_by_account;" 2>&1 | grep -E "(cat_auto|cat_confidence|cat_user|cat_date_user|cat_validée)" | sed 's/^/   │ /'
    echo "   └─────────────────────────────────────────────────────────┘"
else
    warn "⚠️  Certaines colonnes manquantes (trouvé: $COLUMNS/5)"
fi
...
cat > "$REPORT_FILE" << EOF
# 🏗️ Démonstration : Configuration du Schéma DomiramaCatOps

**Date** : $(date +"%Y-%m-%d %H:%M:%S")
**Script** : $(basename "$0")
**Objectif** : Démontrer la création complète du schéma HCD pour DomiramaCatOps
...
Le schéma DomiramaCatOps a été créé avec succès :
...
Voir `10_setup_domiramacatops_poc.sh` (version améliorée) pour un exemple complet de script de setup didactique.
