...
def check_relevance(query: str, results, min_relevance: float = 0.3) -> dict:
    ...
    for result in results:
        ...
        # Calculer le score de pertinence (Jaccard similarity) avec les mots étendus
        ...
        important_keywords = {
            'LOYER', 'IMPAYE', 'VIREMENT', 'PAIEMENT', 'CARTE', 'CB',
            'CARREFOUR', 'RESTAURANT', 'SUPERMARCHE', 'CARTE BLEUE', 'CARTE BANCAIRE'
        }
        has_important_keyword = any(kw in libelle_upper for kw in important_keywords if kw in query_upper or any(syn in query_upper for syn in synonym_mapping.get(kw, [])))
