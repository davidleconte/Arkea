...
def test_cache_embeddings() -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        avg_without = sum(times_without_cache) / len(times_without_cache) if times_without_cache else 0
        ...
        print(f"   ✅ Accélération : {speedup:.2f}x (sans cache: {avg_without:.2f}ms, avec cache: {avg_with:.2f}ms)")
...
def test_cache_invalidation(session, code_si: str, contrat: str) -> Tuple[bool, str, Dict]:
    ...
    try:
        ...
        return True, f"Invalidation validée ({_cache_stats['invalidations']} invalidations)", invalidation_info
    except Exception as e:
        return False, f"Erreur : {e}", {}
