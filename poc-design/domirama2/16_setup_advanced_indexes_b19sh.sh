...
INDEX_COUNT_BEFORE=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE INDEXES;" 2>&1 | grep -v "Warnings" | grep -c "idx_libelle" || echo "0")
...
INDEX_COUNT_AFTER=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE INDEXES;" 2>&1 | grep -v "Warnings" | grep -c "idx_libelle" || echo "0")
...
INDEX_DETAILS=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE INDEX idx_libelle_fulltext_advanced;" 2>&1 | grep -v "Warnings" | head -20)
...
INDEX_DETAILS2=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE INDEX idx_libelle_prefix_ngram;" 2>&1 | grep -v "Warnings" | head -20)
...
COLUMN_DETAILS=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -v "Warnings" | grep "libelle_prefix" || echo "")
