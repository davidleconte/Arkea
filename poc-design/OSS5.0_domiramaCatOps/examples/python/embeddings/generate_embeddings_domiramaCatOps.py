...
def main():
    ...
    partitions = list(
        session.execute("SELECT DISTINCT code_si, contrat FROM operations_by_account")
    )
    ...
    for i in range(0, total, BATCH_SIZE):
        ...
        for row in batch:
            try:
                ...
                print(
                    f"⚠️ Erreur encodage pour {getattr(row,'code_si',None)}/{getattr(row,'contrat',None)}: {e}"
                )
                errors += 1

        # Update sequentially (CQL batch could be considered but beware batch
        # size limits)
        for row, emb in embeddings:
            try:
                # store embedding as list<float>
                session.execute(
                    update_prep, [emb, row.code_si, row.contrat, row.date_op, row.numero_op]
                )
                updated += 1
            except Exception as e:
