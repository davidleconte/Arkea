...
def test_consistency(
    session, tokenizer, model, query: str, code_si: str, contrat: str, iterations: int = 10
):
    ...
    order_consistent = all(
        results_list[i] == results_list[i + 1] for i in range(len(results_list) - 1)
    )
...
def main():
    ...
    # Requêtes de test
    test_queries = ["LOYER IMPAYE", "PAIEMENT CARTE", "VIREMENT", "CARREFOUR"]
    ...
    for query in test_queries:
        ...
        print("   Répétition : 10 fois")
        print()

        consistency = test_consistency(
            session, tokenizer, model, query, code_si, contrat, iterations=10
        )
        ...
        if consistency["all_same"]:
            print(
                f"   ✅ Cohérence OK : Tous les résultats identiques ({consistency['iterations']} itérations)"
            )
        else:
            print("   ⚠️  Incohérence détectée : Résultats différents entre itérations")
            print("   💡 Note : Cela peut être dû à la non-déterminisme du modèle ou aux données")

        if consistency["order_consistent"]:
            print("   ✅ Ordre stable : L'ordre des résultats est cohérent")
        else:
            print("   ⚠️  Ordre instable : L'ordre des résultats varie")

        # Afficher la pertinence
        if relevance["is_mostly_relevant"]:
            print(
                f"   ✅ Pertinence OK : {relevance['relevant_count']}/{relevance['total_count']} résultats pertinents ({relevance['relevance_rate']:.1%})"
            )
        else:
            print(
                f"   ⚠️  Pertinence faible : {relevance['relevant_count']}/{relevance['total_count']} résultats pertinents ({relevance['relevance_rate']:.1%})"
            )
            print(
                "   💡 Note : Les résultats peuvent ne pas être pertinents si les données de test ne contiennent pas de libellés correspondants"
            )

        print("   📊 Résultats (première itération) :")
