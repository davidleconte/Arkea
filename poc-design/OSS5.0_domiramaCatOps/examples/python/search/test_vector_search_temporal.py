...
from test_vector_search_base import connect_to_hcd, encode_text, get_test_account, load_model
        ...
        for row in all_results:
            ...
            if date_start and date_end:
                if not hasattr(row, "date_op") or not row.date_op:
                    continue
                # Gérer différents formats de date_op (timestamp en ms ou
                # datetime)
...
def main():
    ...
    results = vector_search_with_filters(
        session,
        query_embedding,
        code_si,
        contrat,
    ...
    for i, row in enumerate(results[:3], 1):
        libelle = row.libelle[:40] if row.libelle else "N/A"
        date_op = (
            datetime.fromtimestamp(row.date_op / 1000)
            if hasattr(row, "date_op") and row.date_op
            else None
        )
    ...
    print("-" * 70)
    results = vector_search_with_filters(
        session, query_embedding, code_si, contrat, montant_min=100, limit=5
    )
    print(f"   {len(results)} résultat(s)")
    for i, row in enumerate(results[:3], 1):
        libelle = row.libelle[:40] if row.libelle else "N/A"
    ...
    print("-" * 70)
    results = vector_search_with_filters(
        session, query_embedding, code_si, contrat, categorie="HABITATION", limit=5
    )
    print("   Catégorie : HABITATION")
    print(f"   {len(results)} résultat(s)")
    for i, row in enumerate(results[:3], 1):
        libelle = row.libelle[:40] if row.libelle else "N/A"
