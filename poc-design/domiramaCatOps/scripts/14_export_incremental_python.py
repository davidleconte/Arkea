...
#
# UTILISATION :
#   python3 14_export_incremental_python.py [start_date] [end_date] [output_path] [compression] [code_si] [contrat]
#
# PARAMÈTRES :
...
#   output_path : Chemin de sortie (optionnel, défaut: /tmp/exports/domiramaCatOps/incremental_python)
...
def parse_args():
    ...
    parser.add_argument('start_date', nargs='?', default='2024-06-01', help='Date de début (YYYY-MM-DD)')
    parser.add_argument('end_date', nargs='?', default='2024-07-01', help='Date de fin (YYYY-MM-DD)')
    parser.add_argument('output_path', nargs='?', default='/tmp/exports/domiramaCatOps/incremental_python', help='Chemin de sortie')
    parser.add_argument('compression', nargs='?', default='snappy', help='Compression (snappy, gzip, lz4)')
    parser.add_argument('code_si', nargs='?', default=None, help='Code SI pour filtrage (optionnel)')
    parser.add_argument('contrat', nargs='?', default=None, help='Contrat pour filtrage (optionnel)')
...
def get_partitions(session, code_si_filter: Optional[str] = None, contrat_filter: Optional[str] = None) -> List[Tuple[str, str]]:
    ...
    try:
        if code_si_filter and contrat_filter:
            ...
            query = f"SELECT DISTINCT code_si, contrat FROM domiramacatops_poc.operations_by_account WHERE code_si = '{code_si_filter}' ALLOW FILTERING"
            result = session.execute(query)
            partitions = [(row.code_si, row.contrat) for row in result]
        else:
            # Toutes les partitions (peut être lent)
            query = "SELECT DISTINCT code_si, contrat FROM domiramacatops_poc.operations_by_account ALLOW FILTERING"
            result = session.execute(query)
            partitions = [(row.code_si, row.contrat) for row in result]
...
def export_partition(session, code_si: str, contrat: str, start_date: str, end_date: str) -> pd.DataFrame:
    ...
    query = f"""
    SELECT code_si, contrat, date_op, numero_op, libelle, montant, devise, date_valeur, 
           type_operation, sens_operation, operation_data, meta_flags, cat_auto, cat_confidence, 
           cat_user, cat_date_user, cat_validee, libelle_prefix, libelle_tokens, libelle_embedding, 
    ...
    try:
        ...
        for row in result:
            row_dict = {
                ...
                'libelle_embedding': str(row.libelle_embedding) if row.libelle_embedding else None,  # VECTOR en string
...
def main():
    ...
    if args.code_si:
        ...
        print(f"🔍 Filtrage : toutes les partitions")
    print("")
    ...
    for i, (code_si, contrat) in enumerate(partitions, 1):
        ...
        if not df.empty:
            ...
            print(f"   ⚠️  Aucune opération trouvée")
        print("")
    ...
    if all_dataframes:
        ...
        combined_df = combined_df.drop_duplicates(
            subset=['code_si', 'contrat', 'date_op', 'numero_op'], 
            keep='first'
        )
