...
# Script d'installation Spark + Kafka + spark-cassandra-connector (Cross-Platform)
...
if [ -n "${KAFKA_HOME:-}" ] && [ -d "${KAFKA_HOME}" ]; then
    ...
    # Linux : Utiliser le script d'installation Linux
    if [ -f "${SCRIPT_DIR}/02_install_kafka_linux.sh" ]; then
        info "Installation de Kafka via script Linux..."
        bash "${SCRIPT_DIR}/02_install_kafka_linux.sh"
        if [ -d "${INSTALL_DIR}/binaire/kafka" ]; then
            export KAFKA_HOME="${INSTALL_DIR}/binaire/kafka"
            export PATH=$KAFKA_HOME/bin:$PATH
            info "KAFKA_HOME : $KAFKA_HOME"
        fi
    else
        warn "Script d'installation Kafka Linux non trouvé."
        warn "Veuillez installer Kafka manuellement ou utiliser le script 02_install_kafka_linux.sh"
    fi
fi
...
echo "   source .poc-profile"
echo "   # Ou manuellement :"
echo "   export SPARK_HOME=\${SPARK_HOME:-\$(pwd)/binaire/spark-3.5.1}"
echo "   export KAFKA_HOME=\${KAFKA_HOME:-/opt/kafka}"
