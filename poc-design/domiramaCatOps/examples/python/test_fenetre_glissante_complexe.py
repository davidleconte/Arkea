...
def test_fenetre_glissante_avec_chevauchement(session, code_si: str, contrat: str):
    ...
    windows = [
        ...
        # Fenêtre 2 : 15 jours (chevauchement 5 jours)
        ("2024-06-15", "2024-06-30"),
        # Fenêtre 3 : 15 jours (chevauchement 5 jours)
        ("2024-06-25", "2024-07-10"),
    ]
...
def test_fenetre_glissante_sans_chevauchement(session, code_si: str, contrat: str):
    ...
    windows = [
        ...
        # Fenêtre 2 : 15 jours (pas de chevauchement)
        ("2024-06-15", "2024-06-30"),
        # Fenêtre 3 : 15 jours (pas de chevauchement)
...
def test_gestion_frontieres(session, code_si: str, contrat: str):
    ...
    try:
        ...
        if first_date and last_date:
            ...
            print(
                f"   ✅ Fenêtre avant première date : {count_before} opérations (devrait être 0 ou faible)"
            )
            ...
            print(
                f"   ✅ Fenêtre après dernière date : {count_after} opérations (devrait être 0 ou faible)"
            )

            return (
                True,
                f"Première: {first_date.strftime('%Y-%m-%d')}, Dernière: {last_date.strftime('%Y-%m-%d')}",
            )
        else:
