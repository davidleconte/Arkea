...
./bin/cqlsh localhost 9042 -f "$API_FILE" 2>&1 | grep -v "Warnings" || true
...
CORRECTED_SAMPLE=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT cat_user FROM operations_by_account LIMIT 10;" 2>&1 | grep -v "Warnings" | grep -v "cat_user" | grep -v "---" | grep -v "^$" | grep -v "null" | head -1)
...
AUTO_SAMPLE=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; SELECT cat_auto FROM operations_by_account LIMIT 10;" 2>&1 | grep -v "Warnings" | grep -v "cat_auto" | grep -v "---" | grep -v "^$" | grep -v "null" | head -1)
