...
def main():
    ...
    for query in test_queries:
        ...
        if consistency["all_same"]:
            print(
                "   ✅ Cohérence OK : Tous les résultats identiques "
                f"({consistency['iterations']} itérations)"
            )
        else:
            print("   ⚠️  Incohérence détectée : Résultats différents entre itérations")
            print("   💡 Note : Cela peut être dû à la non-déterminisme du modèle")
        ...
        if relevance["is_mostly_relevant"]:
            print(
                "   ✅ Pertinence OK : "
                f"{relevance['relevant_count']}/{relevance['total_count']} "
                f"résultats pertinents ({relevance['relevance_rate']:.1%})"
            )
        else:
            print(
                "   ⚠️  Pertinence faible : "
            ...
            print(
                "   💡 Note : Les résultats peuvent ne pas être pertinents "
                "si les données de test ne contiennent pas de libellés correspondants"
            )
