...
class OperationService:
    ...
    def get_category_at_date(self, operation: Operation, query_date: datetime) -> str:
        """
        ...
        if operation.cat_user and operation.cat_date_user and operation.cat_date_user <= query_date:
    ...
    def get_operation_with_category(
        self, code_si: str, contrat: str, date_op: datetime, numero_op: int, query_date: datetime
    ) -> OperationWithCategory:
        """
        Récupère une opération avec sa catégorie valide à une date donnée
        """
        # 1. Récupérer l'opération depuis HCD
        operation = self.repository.find_by_key(code_si, contrat, date_op, numero_op)

        if operation is None:
            raise ValueError(f"Operation not found: {code_si}/{contrat}/{date_op}/{numero_op}")
