...
def test_multi_models_fusion():
    ...
    for query_text in test_queries:
        ...
        for idx, result in enumerate(merged_results[:5], 1):
            ...
            print(
                f"      {idx}. {result['libelle']} "
                f"(modèles: {models_str}, score: {result['best_score']:.2f})"
            )
    ...
    for summary in results_summary:
        print(
            f"✅ '{summary['query']}' : {summary['merged_count']} résultats fusionnés "
            f"en {summary['search_time']:.3f}s"
        )
