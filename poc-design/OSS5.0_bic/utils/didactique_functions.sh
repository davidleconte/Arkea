...
setup_paths() {
    ...
    # Pour OSS5.0: utiliser Podman containers
    export INSTALL_DIR="${INSTALL_DIR:-$ARKEA_HOME}" # Pour compatibilité avec anciens scripts
    export HCD_DIR="${HCD_DIR:-podman}" # OSS5.0: use Podman
    export SPARK_HOME="${SPARK_HOME:-podman}" # OSS5.0: use Podman
    export HCD_HOST="${HCD_HOST:-localhost}"
    export HCD_PORT="${HCD_PORT:-9102}" # OSS5.0: external port 9102
    export KAFKA_PORT="${KAFKA_PORT:-9192}" # OSS5.0: external port 9192
    export SPARK_MASTER_PORT="${SPARK_MASTER_PORT:-9280}" # OSS5.0: Spark UI
