...
SCHEMA_CHECK=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE TABLE operations_by_account;" 2>&1 | grep -c "libelle_embedding" 2>/dev/null | head -1 || echo "0")
...
if [ "$SCHEMA_CHECK" -eq 0 ] || [ -z "$SCHEMA_CHECK" ]; then
    ...
    INDEX_CHECK=$(./bin/cqlsh localhost 9042 -e "USE domirama2_poc; DESCRIBE INDEX idx_libelle_embedding_vector;" 2>&1 | grep -c "idx_libelle_embedding_vector" 2>/dev/null | head -1 || echo "0")
