...
from test_vector_search_base import (
    load_model, encode_text, vector_search, fulltext_search,
...
def main():
    ...
    for query_text, description in test_queries:
        ...
        print(f"   📊 Vector Search ({comparison['vector']['count']} résultats, {comparison['vector']['time']:.2f} ms):")
        ...
        print(f"   📊 Full-Text Search ({comparison['fulltext']['count']} résultats, {comparison['fulltext']['time']:.2f} ms):")
        ...
        if comparison['fulltext']['count'] == 0 and comparison['vector']['count'] > 0:
            ...
            if comparison['fulltext']['time'] < comparison['vector']['time']:
                print(f"   ⚠️  Full-Text plus rapide ({comparison['fulltext']['time']:.2f} ms vs {comparison['vector']['time']:.2f} ms)")
            else:
                print(f"   ⚠️  Vector plus rapide ({comparison['vector']['time']:.2f} ms vs {comparison['fulltext']['time']:.2f} ms)")
        print()
        print("-" * 70)
        print()
